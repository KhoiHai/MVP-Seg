...
def generate_locations(feature_list, strides):
    ...
    for f, stride in zip(feature_list, strides):
        ...
        shifts_x = torch.arange(0, W * stride, step=stride, device=f.device, dtype=torch.float32) + (float(stride) * 0.5)
        shifts_y = torch.arange(0, H * stride, step=stride, device=f.device, dtype=torch.float32) + (float(stride) * 0.5)
