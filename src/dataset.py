...
# DATASET TESTING
# ------------------------------
def test_dataset(data_root = "data/coco"):
    '''
    Script for testing dataset loading and augmentation pipeline
    '''
    train_loader, val_loader = get_dataloaders(
        data_root   = data_root,
        batch_size  = 2,
        num_workers = 0,          # 0 for easier debugging
        subset_size = 100         # Small subset just for testing
    )

    # Test one batch from train loader
    batch = next(iter(train_loader))
    if batch is None:
        print("Empty batch returned")
        return

    images, targets = batch

    print("===== DATASET TEST =====")
    print(f"Images shape : {images.shape}")       # [B, 3, 550, 550]
    ...
    for i, tgt in enumerate(targets):
        ...
        print(f"    img_id : {tgt['img_id']}")

if __name__ == '__main__':
    test_dataset(data_root = "data/coco")